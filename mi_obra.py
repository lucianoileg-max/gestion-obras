...
st.set_page_config(
    ...
    page_icon="logo_estudio.png" if os.path.exists("logo_estudio.png") else None,
    layout="wide",
...
# Aquí solo añadimos tipografías de marca y detalles, con selectores estables (data-testid).
BRAND_CSS = """
<style>
...
:root{
    ...
    --cream-on-dark:#EDEAE1; --cream-on-dark-soft:#B9B6AC;
...
[data-testid="stWidgetLabel"] p, [data-testid="stMetricLabel"] {
    ...
    color: var(--cream-on-dark-soft) !important;
}
...
.brand-eyebrow{
    ...
    color: var(--oak-soft); text-align:center; margin: 18px 0 0 0; display:flex; align-items:center;
    justify-content:center; gap:10px;
}
.brand-eyebrow::before, .brand-eyebrow::after{ content:''; width:24px; height:1px; background:var(--oak-soft); }
</style>
"""
...
try: cursor.execute("ALTER TABLE obras ADD COLUMN categoria_encargo TEXT DEFAULT 'Obra Completa (Construcción)'")
except sqlite3.OperationalError: pass
...
# Checklist de tareas propias del despacho (independiente del Cronograma de obra)
cursor.execute("CREATE TABLE IF NOT EXISTS checklist_despacho (id INTEGER PRIMARY KEY AUTOINCREMENT, obra_id INTEGER, tipo_encargo TEXT, fase TEXT, tarea TEXT, orden INTEGER, completado INTEGER DEFAULT 0, fecha_completado TEXT)")
try: cursor.execute("ALTER TABLE checklist_despacho ADD COLUMN modo TEXT DEFAULT 'manual'")
except sqlite3.OperationalError: pass
...
# --- PLANTILLAS DE TAREAS POR TIPO DE ENCARGO ---
# Fuente única de verdad: cada tipo de encargo (el mismo que eliges en la Calculadora COAC
# o en el Generador de Cronograma) tiene su propia lista de tareas.
# Formato de cada tarea: (etapa, tarea, pct_inicio, pct_fin, pct_coste, responsable)
#   pct_inicio / pct_fin -> posición dentro de la duración total del encargo (0.0 a 1.0)
#   pct_coste             -> % del presupuesto/honorarios de ESE encargo asignado a la tarea
#                            (las pct_coste de una misma plantilla deberían sumar 1.0)
# Podés editar libremente etapas, tareas, pesos y responsables por defecto: es solo el punto
# de partida que se vuelca al Cronograma, luego cada tarea se puede modificar como cualquier otra.
TIPOS_ENCARGO_TAREAS = {
    ...
    "Obra Nueva Unifamiliar Aislada": [
        ("01. Movimiento de Tierras", "Explanación y excavación de zanjas", 0.00, 0.08, 0.06, "Movimiento de Tierras"),
        ("02. Cimentación y Estructura", "Cimentación, forjados y estructura", 0.05, 0.35, 0.28, "Estructuras"),
        ("03. Cerramientos y Cubierta", "Cerramientos de fachada y cubierta", 0.30, 0.50, 0.16, "Cerramientos"),
        ("04. Instalaciones", "Instalaciones eléctrica, fontanería y climatización", 0.42, 0.72, 0.22, "Instalaciones"),
        ("05. Acabados y Pintura", "Alicatados, solados y pintura", 0.65, 0.90, 0.18, "Acabados"),
        ("06. Carpinterías", "Carpinterías interiores y exteriores", 0.78, 0.95, 0.06, "Carpintería"),
        ("07. Urbanización Exterior", "Urbanización y vallado perimetral", 0.90, 1.00, 0.04, "Urbanización"),
    ],
    "Adecuación de Local Comercial": [
        ("01. Desmontajes", "Desmontaje de instalación anterior", 0.00, 0.10, 0.08, "Demoliciones"),
        ("04. Instalaciones", "Instalaciones eléctrica, climatización y PCI", 0.08, 0.45, 0.32, "Instalaciones"),
        ("03. Cerramientos y Divisiones", "Tabiquería y falsos techos", 0.30, 0.60, 0.18, "Pladur"),
        ("05. Acabados y Pintura", "Pavimentos y acabados murales", 0.50, 0.80, 0.20, "Solados"),
        ("06. Carpinterías", "Carpintería, escaparate y rótulo", 0.70, 0.95, 0.14, "Carpintería"),
        ("07. Legalización", "Legalización de actividad y apertura", 0.85, 1.00, 0.08, "Gestoría"),
    ],
    "Cédula de Habitabilidad": [
        ("01. Trabajo de Campo", "Visita técnica y toma de medidas in situ", 0.00, 0.30, 0.35, "Arquitecto"),
        ("02. Redacción", "Elaboración de croquis y cumplimentación del modelo", 0.25, 0.65, 0.35, "Arquitecto"),
        ("03. Trámite", "Presentación telemática en la Agencia de l'Habitatge", 0.60, 0.90, 0.20, "Gestoría"),
        ("04. Entrega", "Entrega de la cédula al cliente", 0.90, 1.00, 0.10, "Arquitecto"),
    ],
    "Certificado Energético (CEE)": [
        ("01. Trabajo de Campo", "Visita técnica y toma de datos constructivos", 0.00, 0.30, 0.30, "Arquitecto"),
        ("02. Cálculo", "Introducción de datos en herramienta CE3/CE3X", 0.25, 0.60, 0.30, "Arquitecto"),
        ("03. Registro", "Generación de etiqueta y registro en ICAEN", 0.55, 0.90, 0.30, "Gestoría"),
        ("04. Entrega", "Entrega del certificado y etiqueta al cliente", 0.90, 1.00, 0.10, "Arquitecto"),
    ],
    "Asesoramiento / Informe": [
        ("01. Auditoría Previa", "Auditoría previa y viabilidad", 0.00, 0.45, 0.40, "Arquitecto"),
        ("02. Seguimiento", "Seguimiento y control de obra", 0.40, 1.00, 0.60, "Arquitecto"),
    ],
}
 
def insertar_tareas_desde_plantilla(obra_id, nombre_plantilla, fecha_inicio, duracion_dias, presupuesto_base, borrar_previas=False):
    """Genera automáticamente en el Cronograma las tareas correspondientes a un tipo de
    encargo, usando la plantilla de TIPOS_ENCARGO_TAREAS. Es el punto de conexión entre
    la Calculadora COAC y el Gestor de Proyectos."""
    matriz = TIPOS_ENCARGO_TAREAS.get(nombre_plantilla, [])
    if not matriz:
        return
    if borrar_previas:
        cursor.execute("DELETE FROM cronograma WHERE obra_id = ?", (obra_id,))
    for etapa_m, tarea_m, pct_ini, pct_fin, pct_coste, resp_m in matriz:
        f_ini_calc = fecha_inicio + timedelta(days=int(duracion_dias * pct_ini))
        f_fin_calc = fecha_inicio + timedelta(days=max(1, int(duracion_dias * pct_fin)))
        coste_calc = round(presupuesto_base * pct_coste, 2)
        cursor.execute(
            "INSERT INTO cronograma (obra_id, etapa, tarea, fecha_inicio, fecha_fin, coste_estimado, avance_porcentaje, responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (obra_id, etapa_m, tarea_m, str(f_ini_calc), str(f_fin_calc), coste_calc, 0, resp_m)
        )
 
# --- CHECKLIST DE TAREAS DEL DESPACHO (proceso interno del arquitecto, NO la ejecución física de obra) ---
# Basado en el paso a paso metodológico del estudio: cada tipo de encargo tiene sus propias fases
# y tareas de gabinete/gestión. Es independiente del Cronograma (que es la obra física del constructor).
TAREAS_DESPACHO_POR_ENCARGO = {
    "Proyectos de Edificación": [
        ("Fase 1: Anteproyecto", "Toma de datos y reuniones iniciales con el cliente"),
        ("Fase 1: Anteproyecto", "Primeros esquemas y encaje volumétrico"),
        ("Fase 2: Proyecto Básico", "Definición urbanística y justificación normativa"),
        ("Fase 2: Proyecto Básico", "Presentación de la solicitud de licencia de obras ante el Ayuntamiento"),
        ("Fase 3: Proyecto Ejecutivo", "Redacción de memorias constructivas"),
        ("Fase 3: Proyecto Ejecutivo", "Planos de detalle y pliego de condiciones"),
        ("Fase 3: Proyecto Ejecutivo", "Definición de instalaciones completas"),
        ("Fase 4: Licitación", "Comparativa ciega de propuestas económicas de contratas e industriales"),
        ("Fase 4: Licitación", "Selección de la mejor oferta"),
        ("Fase 5: Dirección de Obra", "Seguimiento en obra y anotaciones en el Libro de Órdenes"),
        ("Fase 5: Dirección de Obra", "Emisión de certificaciones mensuales (retención 5% de garantía)"),
        ("Fase 6: Cierre y Posventa", "Emisión del Certificado Final de Obra (CFO)"),
        ("Fase 6: Cierre y Posventa", "Firma del Acta de Recepción (inicia el año de garantía)"),
        ("Fase 6: Cierre y Posventa", "Liberación de la retención del 5% al finalizar la garantía"),
    ],
    "Cédula de Habitabilidad": [
        ("Fase 1: Trabajo de Campo", "Visita a la vivienda: toma de medidas in situ"),
        ("Fase 1: Trabajo de Campo", "Comprobación normativa"),
        ("Fase 2: Gabinete", "Elaboración de croquis"),
        ("Fase 2: Gabinete", "Cumplimentación de la solicitud administrativa"),
        ("Fase 3: Trámite", "Presentación telemática del expediente y pago de tasas"),
        ("Fase 3: Trámite", "Recepción y entrega final al cliente"),
    ],
    "Certificado Energético (CEE)": [
        ("Fase 1: Toma de Datos", "Inspección ocular"),
        ("Fase 1: Toma de Datos", "Recopilación de datos de cerramientos y sistemas térmicos"),
        ("Fase 2: Simulación", "Introducción de geometría y datos técnicos en software homologado (CE3X)"),
        ("Fase 3: Registro", "Generación del informe final"),
        ("Fase 3: Registro", "Registro en el ICAEN para obtener la etiqueta"),
        ("Fase 3: Registro", "Entrega al cliente"),
    ],
    "Informes, Peritajes y Due Diligence": [
        ("Fase 1: Due Diligence Pre-Compra", "Análisis normativo y viabilidad"),
        ("Fase 1: Due Diligence Pre-Compra", "Comprobación de linderos"),
        ("Fase 1: Due Diligence Pre-Compra", "Comprobación de normativas y posibles cargas urbanísticas"),
        ("Fase 2: Seguimiento de Obra", "Representación técnica del cliente comprador"),
        ("Fase 2: Seguimiento de Obra", "Control técnico sobre la obra ejecutada por terceros"),
    ],
    "Servicios Adicionales Subcontratados": [
        ("Fase Única: Gestión Temprana", "Encargo y gestión del Estudio Geotécnico"),
        ("Fase Única: Gestión Temprana", "Encargo del Cálculo de Estructuras"),
        ("Fase Única: Gestión Temprana", "Incorporación de resultados a la definición del Proyecto Ejecutivo"),
    ],
}
 
# Vincula cada tipo de la Calculadora COAC con su checklist de despacho correspondiente
CALC_A_CHECKLIST_DESPACHO = {
    "1. Proyecto Completo (Baremo)": "Proyectos de Edificación",
    "2. Cédula de Habitabilidad": "Cédula de Habitabilidad",
    "3. Certificado Energético (CEE)": "Certificado Energético (CEE)",
    "4. Asesoramiento / Informe": "Informes, Peritajes y Due Diligence",
}
 
# --- AUTO-DETECCIÓN: qué tareas del checklist se pueden verificar cruzando datos reales de la obra ---
# Si una tarea aparece aquí, nace en modo 'auto' (bloqueada, la marca el propio sistema).
# Si no aparece, nace en modo 'manual' (checkbox libre, como hasta ahora).
TAREAS_CON_AUTO_DETECCION = {
    "Proyectos de Edificación": {
        "Toma de datos y reuniones iniciales con el cliente",
        "Presentación de la solicitud de licencia de obras ante el Ayuntamiento",
        "Comparativa ciega de propuestas económicas de contratas e industriales",
        "Selección de la mejor oferta",
        "Emisión de certificaciones mensuales (retención 5% de garantía)",
        "Emisión del Certificado Final de Obra (CFO)",
        "Firma del Acta de Recepción (inicia el año de garantía)",
        "Liberación de la retención del 5% al finalizar la garantía",
    },
    "Cédula de Habitabilidad": {
        "Presentación telemática del expediente y pago de tasas",
        "Recepción y entrega final al cliente",
    },
    "Certificado Energético (CEE)": {
        "Generación del informe final",
        "Registro en el ICAEN para obtener la etiqueta",
        "Entrega al cliente",
    },
    "Informes, Peritajes y Due Diligence": {
        "Análisis normativo y viabilidad",
        "Comprobación de linderos",
        "Comprobación de normativas y posibles cargas urbanísticas",
        "Representación técnica del cliente comprador",
        "Control técnico sobre la obra ejecutada por terceros",
    },
    "Servicios Adicionales Subcontratados": {
        "Encargo y gestión del Estudio Geotécnico",
        "Encargo del Cálculo de Estructuras",
    },
}
 
def tarea_tiene_autodeteccion(tipo_encargo, tarea):
    return tarea in TAREAS_CON_AUTO_DETECCION.get(tipo_encargo, set())
 
def evaluar_autodeteccion(tipo_encargo, tarea, dh, dt, dl, dc, dz, di):
    """Cruza los datos reales ya cargados de la obra (honorarios, trámites, licitaciones,
    certificaciones, cierre, ingeniería) para decidir si una tarea 'auto' ya está hecha.
    Devuelve (completado: bool, detalle: str)."""
 
    def hon_existe(patron, estados=None):
        sub = dh[dh["fase"].str.contains(patron, case=False, na=False)]
        if estados:
            sub = sub[sub["estado"].isin(estados)]
        return not sub.empty
 
    if tipo_encargo == "Proyectos de Edificación":
        if tarea == "Toma de datos y reuniones iniciales con el cliente":
            ok = hon_existe("Visita Inicial")
            return ok, "Honorario 'Visita Inicial y Viabilidad' cargado (Fase 1)" if ok else "Pendiente: aún no se cargó el honorario de Visita Inicial"
        if tarea == "Presentación de la solicitud de licencia de obras ante el Ayuntamiento":
            ok = not dt[dt["tipo_tramite"].isin(["Licencia de Obra Mayor", "Comunicación Previa / Obra Menor"])].empty
            return ok, "Trámite de licencia registrado en Fase 2" if ok else "Pendiente: no hay trámite de licencia registrado"
        if tarea == "Comparativa ciega de propuestas económicas de contratas e industriales":
            ok = not dl.empty
            return ok, f"{len(dl)} licitación(es) cargadas en Fase 3" if ok else "Pendiente: no hay licitaciones cargadas"
        if tarea == "Selección de la mejor oferta":
            ok = not dl[dl["estado"] == "Adjudicado"].empty
            return ok, "Hay una empresa adjudicada en Fase 3" if ok else "Pendiente: ninguna licitación está adjudicada"
        if tarea == "Emisión de certificaciones mensuales (retención 5% de garantía)":
            ok = not dc.empty
            return ok, f"{len(dc)} certificación(es) registradas en Fase 4" if ok else "Pendiente: no hay certificaciones registradas"
        if tarea == "Emisión del Certificado Final de Obra (CFO)":
            ok = (not dz.empty) and dz.iloc[0]["fecha_cfo"] not in (None, "-", "")
            return ok, f"CFO emitido el {dz.iloc[0]['fecha_cfo']}" if ok else "Pendiente: falta fecha de CFO en Fase 5"
        if tarea == "Firma del Acta de Recepción (inicia el año de garantía)":
            ok = (not dz.empty) and dz.iloc[0]["fecha_acta_recepcion"] not in (None, "-", "")
            return ok, f"Acta firmada el {dz.iloc[0]['fecha_acta_recepcion']}" if ok else "Pendiente: falta firma de Acta en Fase 5"
        if tarea == "Liberación de la retención del 5% al finalizar la garantía":
            ok = (not dz.empty) and dz.iloc[0]["retencion_devuelta"] == "Sí (Liquidado)"
            return ok, "Retención devuelta según Fase 5" if ok else "Pendiente: retención aún no liquidada"
 
    elif tipo_encargo == "Cédula de Habitabilidad":
        if tarea == "Presentación telemática del expediente y pago de tasas":
            ok = hon_existe("Cédula de Habitabilidad", estados=["Factura Emitida", "Cobrado"])
            return ok, "Honorario del trámite facturado" if ok else "Pendiente: honorario aún no facturado"
        if tarea == "Recepción y entrega final al cliente":
            ok = hon_existe("Cédula de Habitabilidad", estados=["Cobrado"])
            return ok, "Honorario cobrado (trámite cerrado)" if ok else "Pendiente: honorario aún no cobrado"
 
    elif tipo_encargo == "Certificado Energético (CEE)":
        if tarea in ("Generación del informe final", "Registro en el ICAEN para obtener la etiqueta"):
            ok = hon_existe("Certificación Energética", estados=["Factura Emitida", "Cobrado"])
            return ok, "Honorario del trámite facturado" if ok else "Pendiente: honorario aún no facturado"
        if tarea == "Entrega al cliente":
            ok = hon_existe("Certificación Energética", estados=["Cobrado"])
            return ok, "Honorario cobrado (trámite cerrado)" if ok else "Pendiente: honorario aún no cobrado"
 
    elif tipo_encargo == "Informes, Peritajes y Due Diligence":
        if tarea in ("Análisis normativo y viabilidad", "Comprobación de linderos", "Comprobación de normativas y posibles cargas urbanísticas"):
            ok = hon_existe("Auditoría Previa", estados=["Cobrado"])
            return ok, "Fase de Auditoría Previa cobrada" if ok else "Pendiente: fase de Auditoría Previa aún no cobrada"
        if tarea in ("Representación técnica del cliente comprador", "Control técnico sobre la obra ejecutada por terceros"):
            ok = hon_existe("Seguimiento y Control de Obra", estados=["Cobrado"])
            return ok, "Fase de Seguimiento cobrada" if ok else "Pendiente: fase de Seguimiento aún no cobrada"
 
    elif tipo_encargo == "Servicios Adicionales Subcontratados":
        if tarea == "Encargo y gestión del Estudio Geotécnico":
            ok = (di is not None) and pd.notna(di.get("archivo_geo")) and bool(str(di.get("archivo_geo")).strip())
            return ok, "Archivo del estudio geotécnico cargado" if ok else "Pendiente: no se cargó el archivo geotécnico"
        if tarea == "Encargo del Cálculo de Estructuras":
            ok = (di is not None) and pd.notna(di.get("realizado_por_est")) and bool(str(di.get("realizado_por_est")).strip())
            return ok, "Responsable de cálculo de estructuras cargado" if ok else "Pendiente: no se cargó el responsable de estructuras"
 
    return False, ""
 
def generar_checklist_despacho(obra_id, tipo_encargo, borrar_previas=True):
    """Genera el checklist de tareas propias del despacho (gabinete/gestión) para un tipo
    de encargo. No toca el Cronograma de obra: son dos listas de tareas independientes.
    Las tareas con auto-detección nacen en modo 'auto' (bloqueadas); el resto, en 'manual'."""
    tareas = TAREAS_DESPACHO_POR_ENCARGO.get(tipo_encargo, [])
    if not tareas:
        return
    if borrar_previas:
        cursor.execute("DELETE FROM checklist_despacho WHERE obra_id = ? AND tipo_encargo = ?", (obra_id, tipo_encargo))
    for i, (fase, tarea) in enumerate(tareas):
        modo_inicial = "auto" if tarea_tiene_autodeteccion(tipo_encargo, tarea) else "manual"
        cursor.execute(
            "INSERT INTO checklist_despacho (obra_id, tipo_encargo, fase, tarea, orden, completado, fecha_completado, modo) VALUES (?, ?, ?, ?, ?, 0, '-', ?)",
            (obra_id, tipo_encargo, fase, tarea, i, modo_inicial)
        )
...
def generar_propuesta_pdf(datos_obra, df_honorarios):
    ...
    if not df_honorarios.empty:
        ...
        t_hon.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.HexColor("#EFEDE7")), ('GRID', (0, 0), (-1, -1), 0.5, colors.grey), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('FONTSIZE', (0, 0), (-1, -1), 9), ('ALIGN', (1, 1), (-1, -1), 'RIGHT'), ('BOTTOMPADDING', (0, 0), (-1, -1), 6)]))
        story.append(t_hon)
    else:
...
def generar_informe_coac_pdf(nombre_obra, codigo_obra, cliente, municipio, sup, pem, honorarios, porc, cu):
    ...
    styles = getSampleStyleSheet()
    style_title = ParagraphStyle('DocTitle', parent=styles['Heading1'], fontSize=18, leading=22, textColor=colors.HexColor('#1A1C1D'))
    style_section = ParagraphStyle('SecTitle', parent=styles['Heading2'], fontSize=12, leading=16, textColor=colors.HexColor('#1A1C1D'), spaceBefore=10, spaceAfter=6)
    ...
    data_fin = [
        ...
        [Paragraph("<font color='#7C5A38'><b>TOTAL PROPUESTA (Neto + IVA)</b></font>", style_body_bold), Paragraph(f"<font color='#7C5A38'><b>{eur(total)}</b></font>", style_body_bold)]
    ]
...
def generar_informe_tecnico_pdf(datos_obra, ing_row):
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=30, leftMargin=30, topMargin=30, bottomMargin=30)
    story = [Paragraph("MEMORIA TÉCNICA DE PREDIMENSIONADO", ParagraphStyle("Titulo", parent=getSampleStyleSheet()["Heading1"], fontSize=18, textColor=colors.HexColor("#1A1C1D"), alignment=1)), Spacer(1, 15)]
    ...
    if ing_row is not None:
        ...
        t_geo.setStyle(TableStyle([('BACKGROUND', (0,0), (0,-1), colors.HexColor("#EFEDE7")), ('GRID', (0,0), (-1,-1), 0.5, colors.grey), ('VALIGN', (0,0), (-1,-1), 'MIDDLE')]))
        story.append(t_geo)
    doc.build(story)
    buffer.seek(0)
    return buffer
...
if rol == "Arquitecto":
    with st.sidebar.expander("Crear Nuevo Proyecto"):
        with st.form("form_nueva_obra", clear_on_submit=True):
            ...
            nueva_categoria = st.selectbox("Categoría del Encargo:", ["Obra Completa (Construcción)", "Servicio Puntual (Sin Obra Física)"], help="Obra Completa muestra las 5 fases (licencias, licitación, ejecución, cierre). Servicio Puntual (Cédula, CEE, Informe) solo muestra Honorarios y el Checklist de Despacho.")
            ...
            if st.form_submit_button("Crear Proyecto") and nuevo_nombre.strip() != "":
                try:
                    cursor.execute("INSERT INTO obras (nombre, codigo, presupuesto_total, estado_expediente, honorarios_base, superficie_construida, categoria_encargo) VALUES (?, ?, ?, 'En Curso / Activo', 12000.0, 120.0, ?)", (nuevo_nombre, nuevo_codigo, nuevo_presupuesto, nueva_categoria))
                    conn.commit()
                    st.rerun()
                except sqlite3.IntegrityError:
                    st.sidebar.error("Ya existe un proyecto con ese nombre.")
 
    with st.sidebar.expander("Eliminar Proyecto"):
        obras_borrado_df = pd.read_sql_query("SELECT id, codigo, nombre FROM obras", conn)
        if obras_borrado_df.empty:
            st.caption("No hay proyectos para eliminar.")
        else:
            opciones_borrado = {f"{r['codigo']} - {r['nombre']}": r['id'] for _, r in obras_borrado_df.iterrows()}
            proyecto_a_borrar_txt = st.selectbox("Selecciona el proyecto a eliminar:", list(opciones_borrado.keys()), key="sb_borrar_obra")
            id_a_borrar = opciones_borrado[proyecto_a_borrar_txt]
            st.warning("Esta acción borra el proyecto y TODOS sus datos asociados (honorarios, cronograma, checklist, trámites, etc.). No se puede deshacer.")
            confirmar_nombre = st.text_input("Escribe el nombre exacto del proyecto para confirmar:", key="txt_confirmar_borrado")
            nombre_real = obras_borrado_df[obras_borrado_df["id"] == id_a_borrar].iloc[0]["nombre"]
            if st.button("Eliminar Definitivamente", type="primary", use_container_width=True, key="btn_borrar_obra_def"):
                if confirmar_nombre.strip() == nombre_real:
                    tablas_con_obra_id = ["honorarios", "tramites", "licitaciones", "certificaciones", "cierre_obra", "posventa", "incidencias", "cronograma", "documentos", "anteproyectos", "buzon_cliente", "checklist_despacho", "ingenieria_datos", "usuarios"]
                    for tabla_borrar in tablas_con_obra_id:
                        cursor.execute(f"DELETE FROM {tabla_borrar} WHERE obra_id = ?", (id_a_borrar,))
                    cursor.execute("DELETE FROM obras WHERE id = ?", (id_a_borrar,))
                    conn.commit()
...
if rol == "Arquitecto":
    ...
    with st.sidebar.expander("Calculadora COAC y Trámites", expanded=False):
        ...
        if tipo_calculo == "1. Proyecto Completo (Baremo)":
            ...
            st.markdown("---")
            c_subtipo = st.selectbox("Tipología del Encargo (define las tareas):", list(TIPOS_ENCARGO_TAREAS.keys())[:3], key="coac_subtipo")
            c_duracion_meses = st.number_input("Duración estimada (meses):", min_value=1, max_value=36, value=6, key="coac_duracion")
            ...
            gen_tareas_p1 = st.checkbox("Generar también las tareas de este proyecto en el Cronograma", value=True, key="coac_gen_tareas_p1", help="Reemplaza el cronograma actual de esta obra por la plantilla de tareas correspondiente a la tipología elegida.")
            gen_checklist_p1 = st.checkbox("Generar también el Checklist de Despacho (Anteproyecto → Cierre)", value=True, key="coac_gen_checklist_p1")
            ...
            with col_btn_c1:
                if st.button("Aplicar PEC", use_container_width=True):
                    ...
                    if gen_tareas_p1:
                        insertar_tareas_desde_plantilla(obra_id_activa, c_subtipo, date.today(), c_duracion_meses * 30, round(pec_coac_calc, 2), borrar_previas=True)
                    if gen_checklist_p1:
                        generar_checklist_despacho(obra_id_activa, CALC_A_CHECKLIST_DESPACHO[tipo_calculo], borrar_previas=True)
                    conn.commit()
                    st.rerun()
            with col_btn_c2:
        ...
        # ---------------------------------------------------------
            ...
            dias_cedula = st.number_input("Duración estimada (días):", min_value=1, max_value=90, value=15, key="dias_cedula")
            ...
            gen_tareas_cedula = st.checkbox("Añadir las tareas de este trámite al Cronograma", value=True, key="gen_tareas_cedula")
            gen_checklist_cedula = st.checkbox("Generar también el Checklist de Despacho (Campo → Gabinete → Trámite)", value=True, key="gen_checklist_cedula")
            ...
            if st.button("Añadir a Cuadro de Honorarios", use_container_width=True, key="btn_cedula"):
                ...
                if gen_tareas_cedula:
                    insertar_tareas_desde_plantilla(obra_id_activa, "Cédula de Habitabilidad", date.today(), dias_cedula, total_cedula, borrar_previas=False)
                if gen_checklist_cedula:
                    generar_checklist_despacho(obra_id_activa, CALC_A_CHECKLIST_DESPACHO[tipo_calculo], borrar_previas=True)
                conn.commit()
                st.rerun()
        ...
        # ---------------------------------------------------------
            ...
            dias_cee = st.number_input("Duración estimada (días):", min_value=1, max_value=90, value=10, key="dias_cee")
            ...
            gen_tareas_cee = st.checkbox("Añadir las tareas de este trámite al Cronograma", value=True, key="gen_tareas_cee")
            gen_checklist_cee = st.checkbox("Generar también el Checklist de Despacho (Datos → Simulación → Registro)", value=True, key="gen_checklist_cee")
            ...
            if st.button("Añadir a Cuadro de Honorarios", use_container_width=True, key="btn_cee"):
                ...
                if gen_tareas_cee:
                    insertar_tareas_desde_plantilla(obra_id_activa, "Certificado Energético (CEE)", date.today(), dias_cee, total_cee, borrar_previas=False)
                if gen_checklist_cee:
                    generar_checklist_despacho(obra_id_activa, CALC_A_CHECKLIST_DESPACHO[tipo_calculo], borrar_previas=True)
                conn.commit()
                st.rerun()
       ...
       # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
            ...
            st.caption("Cálculo por bolsa de horas (Fraccionado en 2 etapas)")
            
            # 1. Cálculo por horas (mantiene tu precisión)
            horas_est = st.number_input("Horas estimadas de trabajo:", min_value=1.0, value=20.0, step=1.0)
            precio_hora = st.number_input("Precio / Hora (€):", min_value=20.0, value=65.0, step=5.0)
            
            total_asesoria = horas_est * precio_hora
            st.success(f"**Honorarios Base Sugeridos:** {total_asesoria:,.2f} €")
            
            # 2. Selector de porcentajes para la facturación
            st.markdown("**Fraccionamiento del cobro:**")
            col_f1, col_f2 = st.columns(2)
            with col_f1:
                pct_etapa1 = st.number_input("% Auditoría Previa:", min_value=0.0, max_value=100.0, value=40.0)
            with col_f2:
                pct_etapa2 = st.number_input("% Seguimiento Obra:", min_value=0.0, max_value=100.0, value=60.0)
 
            duracion_asesoria_meses = st.number_input("Duración total estimada (meses):", min_value=1, max_value=36, value=6, key="duracion_asesoria")
            gen_tareas_asesoria = st.checkbox("Añadir las tareas de este asesoramiento al Cronograma", value=True, key="gen_tareas_asesoria")
            gen_checklist_asesoria = st.checkbox("Generar también el Checklist de Despacho (Due Diligence → Seguimiento)", value=True, key="gen_checklist_asesoria")
 
            # 3. Botón para volcar al cuadro general
            if st.button("Añadir a Cuadro de Honorarios", use_container_width=True, key="btn_asesoria"):
                if (pct_etapa1 + pct_etapa2) != 100.0:
                    st.error("¡Atención! La suma de los porcentajes debe ser exactamente 100%.")
                else:
                    # Nombres en castellano transparente para el cliente
                    fases_asesoria = [
                        ("01. Auditoría Previa (Viabilidad de Compra)", pct_etapa1, total_asesoria * (pct_etapa1 / 100.0)),
                        ("02. Seguimiento y Control de Obra", pct_etapa2, total_asesoria * (pct_etapa2 / 100.0))
                    ]
                    
                    for nom_fase, pct_fase, base_fase in fases_asesoria:
                        iva_fase = round(base_fase * 0.21, 2)
                        irpf_fase = round(base_fase * 0.15, 2)
                        total_factura = round(base_fase + iva_fase - irpf_fase, 2)
                        
                        cursor.execute("INSERT INTO honorarios (obra_id, fase, porcentaje, base_imponible, iva, retencion_irpf, total_a_cobrar, estado, fecha_emision, fecha_cobro) VALUES (?, ?, ?, ?, ?, ?, ?, 'Pendiente', '-', '-')", 
                                       (obra_id_activa, nom_fase, pct_fase, base_fase, iva_fase, irpf_fase, total_factura))
 
                    if gen_tareas_asesoria:
                        insertar_tareas_desde_plantilla(obra_id_activa, "Asesoramiento / Informe", date.today(), duracion_asesoria_meses * 30, total_asesoria, borrar_previas=False)
                    if gen_checklist_asesoria:
                        generar_checklist_despacho(obra_id_activa, CALC_A_CHECKLIST_DESPACHO[tipo_calculo], borrar_previas=True)
...
# ==========================================
categoria_obra_actual = datos_obra.get("categoria_encargo", "Obra Completa (Construcción)")
if pd.isna(categoria_obra_actual) or not categoria_obra_actual:
    categoria_obra_actual = "Obra Completa (Construcción)"
 
if categoria_obra_actual == "Obra Completa (Construcción)":
    tab_fase1, tab_fase2, tab_fase3, tab_fase4, tab_fase5, tab_checklist = st.tabs([
        "FASE 1: Viabilidad y Anteproyecto",
        "FASE 2: Licencias y Trámites",
        "FASE 3: Licitación y Contratas",
        "FASE 4: Ejecución y Dirección de Obra",
        "FASE 5: Cierre, Finiquito y Posventa",
        "CHECKLIST: Tareas del Despacho"
    ])
else:
    tab_fase1, tab_checklist = st.tabs([
        "Datos y Honorarios del Encargo",
        "CHECKLIST: Tareas del Despacho"
    ])
    tab_fase2 = tab_fase3 = tab_fase4 = tab_fase5 = None
            ...
            with chat_container:
                if not df_buzon.empty:
                    for _, r_msg in df_buzon.iterrows():
                        ...
                        with c_del:
                            if rol == "Arquitecto":
                                if st.button("", key=f"del_msg_{r_msg['id']}", help="Eliminar"):
        ...
        with st.expander("1.2 Presentación de Anteproyecto (Visor Seguro)", expanded=False):
            ...
            if not df_anteproyectos.empty:
                ...
                if rol == "Arquitecto" and est_exp == "En Curso / Activo":
                    with st.expander("Eliminar Anteproyecto"):
                        ...
                        if st.button("Eliminar Definitivamente", key="btn_del_ant"):
            ...
            if ing_row is not None:
                ...
                with col_f1:
                    if pd.notna(ing_row["archivo_geo"]) and ing_row["archivo_geo"] and os.path.exists(ing_row["archivo_geo"]):
                        with open(ing_row["archivo_geo"], "rb") as f: st.download_button("Descargar PDF Geotécnico", f, file_name=os.path.basename(ing_row["archivo_geo"]), key="dl_geo")
                with col_f2:
                    if pd.notna(ing_row["archivo_est"]) and ing_row["archivo_est"] and os.path.exists(ing_row["archivo_est"]):
                        with open(ing_row["archivo_est"], "rb") as f: st.download_button("Descargar PDF Estructuras", f, file_name=os.path.basename(ing_row["archivo_est"]), key="dl_est")
                with col_f3:
                ...
                if est_exp == "En Curso / Activo":
                    ...
                    with st.form("form_estado_cobro", clear_on_submit=False):
                        ...
                        if st.form_submit_button("Actualizar Cobro", use_container_width=True):
                            ...
                            conn.commit()
                            st.rerun()
                    if st.button("Eliminar Fase Seleccionada", type="primary", use_container_width=True, key="btn_del_fase"):
...
# ---------------------------------------------------------
if tab_fase2 is not None:
    with tab_fase2:
        st.markdown("### Fase 2: Licencias, Visados y Requerimientos Administrativos")
                ...
                with st.expander("Actualizar o Eliminar Expediente"):
                    ...
                    with st.form("form_edit_tramite"):
                        ...
                        if cb1.form_submit_button("Guardar"):
                            ...
                            conn.commit()
                            st.rerun()
                        if est_exp == "En Curso / Activo" and cb2.form_submit_button("Eliminar"):
                    ...
                    with st.expander("Adjudicar Oferta Ganadora"):
                        ...
                        with st.form("form_adjudicar_lic"):
                            ...
                            if col_adj1.form_submit_button("Adjudicar"):
                                ...
                                conn.commit()
                                st.rerun()
                            if est_exp == "En Curso / Activo" and col_adj2.form_submit_button("Eliminar"):
    ...
    with tab_fase4:
        ...
        with subtab_gantt:
            if not df_gantt.empty:
                ...
                g2.metric("Avance Físico Global", f"{pct_fisico_global:.1f}%", f"{total_fisico_euros:,.2f} € ejecutados")
            ...
            if not retrasadas.empty or desvio_presupuesto:
                ...
                if desvio_presupuesto:
                    ...
                    if diferencia > 0:
                        st.warning(f"**Desvío:** El total programado supera el presupuesto en +{diferencia:,.2f} €.")
                    else:
                ...
                with st.expander("Generador Automático de Cronograma", expanded=df_gantt.empty):
                    with st.form("form_autoplan"):
                        ...
                        if st.form_submit_button("Generar Cronograma Automático"):
                            insertar_tareas_desde_plantilla(obra_id_activa, tipo_obra, fecha_inicio_auto, meses_obra * 30, datos_obra["presupuesto_total"], borrar_previas=True)
                            conn.commit()
                            st.rerun()
                            ...
                            with st.form("form_editar_partida"):
                                ...
                                if st.form_submit_button("Guardar Cambios"):
                                    if edit_tarea.strip() != "" and edit_f_fin >= edit_f_inicio:
                                        ...
                                        st.rerun()
                        if not df_gantt.empty:
                            if st.button("Eliminar Partida Seleccionada", use_container_width=True, type="primary"):
                    ...
                    with st.expander("Modificar Estado / Eliminar"):
                        ...
                        with st.form("form_edit_cert"):
                            ...
                            if cb_c1.form_submit_button("Guardar"):
                                ...
                                conn.commit()
                                st.rerun()
                            if est_exp == "En Curso / Activo" and cb_c2.form_submit_button("Eliminar"):
        ...
        with subtab_docs:
            ...
            with col_d2:
                ...
                if not df_docs.empty and rol == "Arquitecto" and est_exp == "En Curso / Activo":
                    with st.expander("Eliminar Plano / Documento del Historial"):
                        ...
                        if st.button("Eliminar Documento", key="btn_del_doc"):
                        ...
                        if r_d["archivo_path"] and os.path.exists(r_d["archivo_path"]):
                            ...
                            if rol == "Cliente":
                                if r_d["archivo_path"].lower().endswith(".pdf"):
                                    with st.expander(f"Ver {r_d['codigo_plano']} (Modo Lectura)"):
                                ...
                                with col_btn1:
                                    st.download_button(label=f"Descargar {r_d['codigo_plano']}", data=bytes_data, file_name=os.path.basename(r_d["archivo_path"]), key=f"btn_dl_{r_d['id']}")
                                with col_btn2:
                                    if r_d["archivo_path"].lower().endswith(".pdf"):
                                        with st.expander("Abrir Visor Integrado"):
                                            b64_pdf = base64.b64encode(bytes_data).decode('utf-8')
...
if tab_fase5 is not None:
    with tab_fase5:
        ...
        with col_c2:
            ...
            if row_cierre and row_cierre["fecha_acta_recepcion"] != "-" and row_cierre["estado_cierre"] != "En Ejecución (Obra Abierta)":
                ...
                if dias_restantes > 0:
                    st.info(f"**Garantía Activa:** Quedan **{dias_restantes} días** para cumplir el año legal (Vence: {f_vencimiento_garantia.strftime('%d/%m/%Y')}).")
                else:
        ...
        with col_pv1:
            if est_exp == "En Curso / Activo" and rol in ["Arquitecto", "Cliente"]:
                st.markdown("#### Registrar Reclamación Posventa")
        ...
        with col_pv2:
            ...
            if not df_posventa.empty:
                ...
                if est_exp == "En Curso / Activo" and rol == "Arquitecto":
                    with st.expander("Resolver Incidencia Posventa"):
                        ...
                        with st.form("form_resolver_pv"):
                            ...
                            if cpv1.form_submit_button("Guardar"):
                                ...
                                conn.commit()
                                st.rerun()
                            if cpv2.form_submit_button("Eliminar"):
    ...
    # ---------------------------------------------------------
    # CHECKLIST DEL DESPACHO (tareas de gabinete/gestión, independientes del Cronograma de obra)
    # ---------------------------------------------------------
with tab_checklist:
    st.markdown("#### Checklist de Tareas del Despacho")
    st.caption("Estas tareas son el proceso interno del arquitecto (gabinete, trámites, gestión). No son las tareas físicas de construcción, que se gestionan en el Cronograma de la Fase 4.")
 
    df_checklist = pd.read_sql_query("SELECT * FROM checklist_despacho WHERE obra_id = ? ORDER BY tipo_encargo, orden", conn, params=(obra_id_activa,))
 
    if rol == "Arquitecto":
        with st.expander("Generar / Regenerar Checklist manualmente", expanded=df_checklist.empty):
            with st.form("form_gen_checklist_manual"):
                tipo_encargo_manual = st.selectbox("Tipo de Encargo:", list(TAREAS_DESPACHO_POR_ENCARGO.keys()))
                if st.form_submit_button("Generar Checklist"):
                    generar_checklist_despacho(obra_id_activa, tipo_encargo_manual, borrar_previas=True)
                    conn.commit()
                    st.rerun()
 
    if df_checklist.empty:
        st.info("Todavía no hay checklist generado para esta obra. Generalo desde la Calculadora COAC (sidebar) o manualmente arriba.")
    else:
        total_tareas = len(df_checklist)
        completadas = int(df_checklist["completado"].sum())
        st.progress(completadas / total_tareas if total_tareas else 0)
        st.caption(f"**{completadas} / {total_tareas}** tareas completadas")
 
        for tipo_enc in df_checklist["tipo_encargo"].unique():
            st.markdown(f"### {tipo_enc}")
            df_tipo = df_checklist[df_checklist["tipo_encargo"] == tipo_enc]
            for fase_nombre in df_tipo["fase"].unique():
                st.markdown(f"**{fase_nombre}**")
                df_fase = df_tipo[df_tipo["fase"] == fase_nombre]
                for _, row_t in df_fase.iterrows():
                    modo_actual = row_t.get("modo", "manual") or "manual"
                    tiene_auto = tarea_tiene_autodeteccion(tipo_enc, row_t["tarea"])
 
                    if modo_actual == "auto" and tiene_auto:
                        # --- Tarea auto-vinculada: bloqueada, la marca el propio sistema ---
                        auto_ok, detalle = evaluar_autodeteccion(tipo_enc, row_t["tarea"], df_honorarios, df_tramites, df_licit, df_cert, df_cierre, (df_ing.iloc[0] if not df_ing.empty else None))
                        if bool(row_t["completado"]) != auto_ok:
                            nueva_fecha = str(date.today()) if auto_ok else "-"
                            cursor.execute("UPDATE checklist_despacho SET completado = ?, fecha_completado = ? WHERE id = ?", (1 if auto_ok else 0, nueva_fecha, row_t["id"]))
                            conn.commit()
                        col_chk, col_lock = st.columns([5, 1])
                        with col_chk:
                            st.checkbox(row_t["tarea"], value=auto_ok, key=f"chk_desp_{row_t['id']}", disabled=True)
                            st.caption(f"🔗 {detalle}")
                        with col_lock:
                            if rol == "Arquitecto":
                                if st.button("🔓", key=f"btn_unlock_{row_t['id']}", help="Desbloquear y editar esta tarea a mano"):
                                    cursor.execute("UPDATE checklist_despacho SET modo = 'manual' WHERE id = ?", (row_t["id"],))
                                    conn.commit()
                                    st.rerun()
                    else:
                        # --- Tarea manual (o auto desbloqueada a mano) ---
                        marcado = st.checkbox(row_t["tarea"], value=bool(row_t["completado"]), key=f"chk_desp_{row_t['id']}")
                        if marcado != bool(row_t["completado"]):
                            nueva_fecha = str(date.today()) if marcado else "-"
                            cursor.execute("UPDATE checklist_despacho SET completado = ?, fecha_completado = ? WHERE id = ?", (1 if marcado else 0, nueva_fecha, row_t["id"]))
                            conn.commit()
                            st.rerun()
                        if tiene_auto and rol == "Arquitecto":
                            if st.button("🔒 Re-vincular automáticamente", key=f"btn_relock_{row_t['id']}"):
                                cursor.execute("UPDATE checklist_despacho SET modo = 'auto' WHERE id = ?", (row_t["id"],))
                                conn.commit()
                                st.rerun()
 
            if rol == "Arquitecto":
                if st.button(f"Eliminar checklist de '{tipo_enc}'", key=f"btn_del_checklist_{tipo_enc}"):
                    cursor.execute("DELETE FROM checklist_despacho WHERE obra_id = ? AND tipo_encargo = ?", (obra_id_activa, tipo_enc))
                    conn.commit()
                    st.rerun()
            st.markdown("---")
